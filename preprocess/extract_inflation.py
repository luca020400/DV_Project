...
def extract_inflation_data(input_csv, output_json, country_name=None):
    try:
        with open(input_csv, "r", encoding="utf-8") as csvfile:
            ...
            for row in reader:
                ...
                for year in year_columns:
                    ...
                    try:
                        inflation_value = float(value)
                        inflation_data.append(
                            {"year": f"{year}-01-01", "inflation": inflation_value}
                        )
                    except ValueError:
                ...
                if inflation_data:
                    data_by_country[country] = inflation_data
        ...
        if isinstance(output_data, list):
            print(
                f"✓ Extracted {len(output_data)} inflation data points for {country_name}"
            )
            print(
            ...
            print(
                f"✓ Extracted {len(output_data)} countries with {total_points} inflation data points"
            )
...
if __name__ == "__main__":
    ...
    success = extract_inflation_data(input_file, output_file, country)

...
def extract_exchange_rate_data(input_csv, output_json, country_name=None):
    try:
        with open(input_csv, "r", encoding="utf-8") as csvfile:
            ...
            for row in reader:
                # Only process exchange rate rows
                if row.get("Indicator Name") != "Official exchange rate (LCU per US$, period average)":
                    continue
                ...
                for year in year_columns:
                    ...
                    try:
                        ...
                        exchange_data.append(
                            {"year": f"{year}-01-01", "currency": exchange_value}
                        )
                    except ValueError:
                ...
                if exchange_data:
                    data_by_country[country] = exchange_data
        ...
        if isinstance(output_data, list):
            print(
                f"✓ Extracted {len(output_data)} exchange rate data points for {country_name}"
            )
            print(
            ...
            print(
                f"✓ Extracted {len(output_data)} countries with {total_points} exchange rate data points"
            )
...
if __name__ == "__main__":
    ...
    success = extract_exchange_rate_data(input_file, output_file, country)
